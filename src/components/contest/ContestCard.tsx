...
const ContestCard: React.FC<ContestData> = ({
    id,
    contest,
    company,
    thumbnail,
    ...
    return (
        <>
            ...
            <ContestCardLink
                href={`/contest/${id}`}
                className={thumbnail ? "col-lg-6" : "col-lg-12"}
            >
                <ContestTitle>{contest}</ContestTitle>
                <ContestDetail>주최/주관 : {company}</ContestDetail>
                <ContestDetail>
                    접수기간 : {dateFormat(startDate)} ~ {dateFormat(endDate)}
                    <Badge $variant={isDeadline(endDate) ? "secondary" : "grey"}>
                        {remainingDeadline(endDate)}
                    </Badge>
                </ContestDetail>
